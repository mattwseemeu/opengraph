	...
	static private function _parse($HTML) {
		...
		foreach ($tags AS $tag) {
			if ($tag->hasAttribute('property') &&
				 ...
				 *	The new array will contain the previously held value and the value currently being stored.
				 *
				 *	Subsequent items are pushed into the array.
				 *
				 *	Items that appear once remain as a key holding a string value.
				 */
				if ( isset($page->_values[$key]) && !is_array($page->_values[$key]) ) {
					$page->_values[$key] = array( $page->_values[$key], $tag->getAttribute('content') );
				} elseif ( isset($page->_values[$key]) && is_array($page->_values[$key]) ) {
					array_push( $page->_values[$key], $tag->getAttribute('content') );
				} else {
